...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const sqlStatements = [
      ...
      `DO $$ BEGIN
        ...
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'timecards' AND policyname = 'Admins read all timecards') THEN
          CREATE POLICY "Admins read all timecards" ON public.timecards FOR SELECT USING (has_role(auth.uid(), 'admin'::app_role));
        END IF;
        IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'timecards' AND policyname = 'Admins update all timecards') THEN
          CREATE POLICY "Admins update all timecards" ON public.timecards FOR UPDATE USING (has_role(auth.uid(), 'admin'::app_role));
        END IF;
      END $$`,
    ];

    // Extract project ref from URL
    const projectRef = new URL(extUrl).hostname.split(".")[0];
    ...
    return new Response(JSON.stringify({ 
      success: true,
      message: "Table creation SQL ready. Run on external DB.",
