...
const TIPOS_ENTRADA = [
  { value: "ROTA_AUTOMATICA", label: "Rota (R$10)" },
  { value: "FIXO_ML", label: "Fixo Mercado Livre" },
  { value: "MANUAL", label: "Manual" },
];

const TIPOS_SAIDA = [
  { value: "FUNCIONARIO_DIARIA", label: "Funcionário (diária)" },
...
const AdminFinanceiro = () => {
  ...
  // Calculate working days (Mon-Sat) in the month
  const daysInMonth = getDaysInMonth(new Date(mesRef + "-01"));
  let workingDays = 0;
  for (let d = 1; d <= daysInMonth; d++) {
    const day = getDay(new Date(Number(mesRef.split("-")[0]), Number(mesRef.split("-")[1]) - 1, d));
  ...
  const toggleStatusEntrada = async (item: any) => {
    ...
    await supabase.from("financeiro_entradas").update({
      status: newStatus,
      recebido_em: newStatus === "recebido" ? new Date().toISOString() : null,
    }).eq("id", item.id);
  ...
  const startEditEntrada = (item: any) => {
    ...
    setFormData(item.data_referencia);
    setFormObs(item.observacao || "");
    setEditingId(item.id);
    setShowFormEntrada(true);
    setShowFormSaida(false);
    setTab("entradas");
  };
  ...
  const startEditSaida = (item: any) => {
    ...
    setFormObs(item.observacao || "");
    setEditingId(item.id);
    setShowFormSaida(true);
    setShowFormEntrada(false);
    setTab("saidas");
  };

  // Auto-generate route entries for the month
  const handleGerarRotas = async () => {
    const { data: dias } = await supabase.from("dias").select("id, data")
      .gte("data", mesInicio).lte("data", mesFim);
    if (!dias || dias.length === 0) { toast.error("Nenhum dia operacional neste mês."); return; }

    let totalRotas = 0;
    for (const dia of dias) {
      const { count } = await supabase.from("rotas").select("id", { count: "exact", head: true })
        .eq("dia_id", dia.id).eq("status", "Saída registrada (NX)");
      totalRotas += count || 0;
    }

    if (totalRotas === 0) { toast.error("Nenhuma rota com saída registrada."); return; }

    const { error } = await supabase.from("financeiro_entradas").insert({
      tipo: "ROTA_AUTOMATICA",
      descricao: `${totalRotas} rotas (R$10/rota) — ${format(new Date(mesInicio), "MMM/yyyy", { locale: ptBR })}`,
      valor: totalRotas * 10,
      data_referencia: mesFim,
    });

    if (error) toast.error(error.message);
    else { toast.success(`Entrada gerada: ${totalRotas} rotas × R$10 = R$${(totalRotas * 10).toFixed(2)}`); await loadData(); }
  };
    ...
    <div className="space-y-4 max-w-2xl mx-auto">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            ...
            <Card><CardContent className="pt-4 text-center">
              ...
              <p className={`text-lg font-bold ${saldo >= 0 ? "text-green-600" : "text-red-500"}`}>R${saldo.toFixed(2)}</p>
              <p className="text-[11px] text-muted-foreground">Saldo</p>
            </CardContent></Card>
          </div>

          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-sm">Projeção Mensal</CardTitle></CardHeader>
            <CardContent className="space-y-1 text-sm">
              <div className="flex justify-between"><span className="text-muted-foreground">Dias úteis (seg-sáb):</span><span className="font-medium">{workingDays}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Fixo ML mensal:</span><span className="font-medium text-green-600">R$3.500,00</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Receita média/dia:</span><span className="font-medium">{workingDays > 0 ? `R$${(totalEntradas / Math.max(1, entradas.length > 0 ? workingDays : 1)).toFixed(2)}` : "—"}</span></div>
              <div className="flex justify-between"><span className="text-muted-foreground">Despesa média/dia:</span><span className="font-medium">{workingDays > 0 ? `R$${(totalSaidas / Math.max(1, workingDays)).toFixed(2)}` : "—"}</span></div>
              <div className="flex justify-between pt-2 border-t"><span className="font-medium">Resultado projetado:</span><span className={`font-bold ${saldo >= 0 ? "text-green-600" : "text-red-500"}`}>R${saldo.toFixed(2)}</span></div>
            </CardContent>
          </Card>

          <Button variant="outline" className="w-full" onClick={handleGerarRotas}>
            <TrendingUp className="h-4 w-4 mr-2" /> Gerar Entradas Automáticas (Rotas do Mês)
          </Button>
        </div>
      )}
      ...
      {tab === "entradas" && (
        <div className="space-y-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <Badge variant="outline" className={`text-[10px] cursor-pointer ${item.status === "recebido" ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700"}`}
                    ...
                    {item.status === "recebido" ? "Recebido" : "Aguardando"}
                  </Badge>
          ...
          {entradas.length === 0 && <p className="text-sm text-muted-foreground text-center py-6">Nenhuma entrada neste mês.</p>}
        </div>
      )}
