...
import { LayoutDashboard, CalendarPlus, Route, UserCheck, AlertTriangle, Users, Tv, LogOut, Menu, X, ChevronLeft } from "lucide-react";
const navItems = [{
  to: "/admin/dashboard",
  label: "Dashboard",
...
const AdminLayout = () => {
  ...
  return <div className="min-h-screen flex bg-muted">
      {/* Mobile overlay */}
      ...
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-sidebar-background text-sidebar-foreground flex flex-col transition-transform duration-200 lg:translate-x-0 lg:static ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <nav className="flex-1 py-4 space-y-1 overflow-y-auto bg-secondary-foreground border">
          ...
          return <Link key={item.to} to={item.to} onClick={() => setSidebarOpen(false)} className={`flex items-center gap-3 mx-2 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${isActive ? "bg-sidebar-accent text-sidebar-primary" : "text-sidebar-foreground/80 hover:bg-sidebar-accent/50 hover:text-sidebar-foreground"}`}>
